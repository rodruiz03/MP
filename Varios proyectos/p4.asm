...
sh_push_op:
    ...
    sb t0, 0(t1)
    addi s3, s3, 1
    # Usamos t0 en lugar de t9
    addi s0, s0, 8
    j sh_loop  # Cambiado de sh_continue a sh_loop para avanzar correctamente
...
eval_operate:
    ...
    addi s3, s3, -4       # Retroceder 4 bytes
    add t2, s2, s3
    lw t3, 0(t2)          # Segundo operando
    addi s3, s3, -4       # Retroceder 4 bytes más
    add t2, s2, s3
...
# Truncar el salto de línea al final de una cadena
truncate_newline:
    mv t0, a0
truncate_loop:
    lb t1, 0(t0)
